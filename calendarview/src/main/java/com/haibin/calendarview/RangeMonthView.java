...
public abstract class RangeMonthView extends BaseMonthView {
    ...
    private void draw(Canvas canvas, Calendar calendar, int i, int j,int d) {
        ...
        if (isClisck){
            ...
            isClisck = true;
            mDelegate.mInnerListener.onMonthDateSelected(mDelegate.getCurrentDay(), true);
        }
        if (hasScheme) {
            //标记的日子
...
//        if (calendar.getDay()==mDelegate.getCurrentDay().getDay()){
//            onDrawSelected(canvas, calendar, x, y, false);
//        }
...
////        mCurrentDate.setCurrentDay(false);
//            test(mDelegate.mIndexCalendar);
//            test(mCurrentDate);
