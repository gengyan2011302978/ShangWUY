...
public final class ManifestParser {
    ...
    public List<ConfigModule> parse() {
        ...
        return sortModule(modules);
    }

    /**
     * 根据配置的初始化 {@link ConfigModule#configModulePriority()} 参数对初始顺序进行处理
     *
     * @param modules
     * @return
     */
    private List<ConfigModule> sortModule(List<ConfigModule> modules) {
        //表明当前只有 app module
        if (modules.size() == 1) {
            return modules;
        }
        ...
        if (modules.size() > 1) {
            for (int j = 1; j < modules.size(); j++) {
                ConfigModule key = modules.get(j);
                int i = j - 1;
                while (i >= 0 && key.configModulePriority() < modules.get(i).configModulePriority()) {
                    modules.set(i + 1, modules.get(i));
                    i--;
                }
                modules.set(i + 1, key);
            }
