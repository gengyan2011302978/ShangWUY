...
//    }
    ...
    public static String linearNmber(String linearStr) {
        if ("∞".equals(linearStr)){
            return "∞";
        }
        if (linearStr == null || "0".equals(linearStr)) {
            return "0";
        }
        BigDecimal big = new BigDecimal(linearStr);
        NumberFormat nf = NumberFormat.getInstance();
        linearStr = nf.format(big.setScale(3, BigDecimal.ROUND_DOWN)) + "";
        if (linearStr.contains(".")) {
            return (big.setScale(3, BigDecimal.ROUND_DOWN) + "").replace(",", "");
        } else {
            return linearStr.replace(",", "");
        }
    }
}
