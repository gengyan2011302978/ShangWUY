...
import static com.phjt.shangxueyuan.mvp.ui.fragment.VideoDownloadFragment.mDeleteVideoBeanList;
 ...
 * description: 消息
 */
public class DownloadAdapter extends BaseQuickAdapter<FileLevelBean, BaseViewHolder> {
    public DownloadAdapter(Context context, @Nullable List<FileLevelBean> data) {
        super(R.layout.video_download_adapter, data);
    }
    ...
    protected void convert(BaseViewHolder helper, FileLevelBean item) {
        ...
        helper.setText(R.id.tv_number,item.getActivityTotal()+"个视频");
        ...
        cb_click.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                if (isChecked){
                    mDeleteVideoBeanList.add(item);
                }else {
                    mDeleteVideoBeanList.remove(item);
                }
            }
        });
        if (mDeleteVideoBeanList.contains(item)){
