...
import com.phjt.shangxueyuan.interf.IClickContentCallback;
...
public class FolderTextView extends androidx.appcompat.widget.AppCompatTextView {
    ...
    private String DEFAULT_UNFOLD_TEXT = "阅读全文";
    /**
     * 默认固定行数
     */

    private static final int DEFAULT_FOLD_LINE = 5;
    /**
     * 默认收起和展开文字颜色
     */
     ...
     * 有item 点击中间的内容的回调
     */
    private IClickContentCallback callback;
     ...
     * 内容点击处理
     */
    private ClickableSpan clickSpanContent = new ClickableSpan() {
        @Override
        public void onClick(View widget) {
            //LogUtils.e("666clickSpanContent");
        }
    };


    public void setCallBack(IClickContentCallback callbacks) {
        callback = callbacks;
    }
     ...
     * @return
     */
    public boolean isClick(boolean isClick) {
        return isClicks = isClick;
    }
    ...
    public FolderTextView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mTailColor = a.getColor(R.styleable.FolderTextView_tailTextColor, ContextCompat.getColor(context, R.color.color_1374FF));
        ...
        if (!mIsFold) {
            ...
            if (line < layout.getLineCount()) {
                ...
                if (index > 0) {
                    ...
                    //LogUtils.d(TAG, "strWhichHasExactlyFoldLine-->" + strWhichHasExactlyFoldLine);
    ...
    protected void onDraw(Canvas canvas) {
        //LogUtils.d(TAG, "onDraw() " + mCountOnDraw++ + ", getMeasuredHeight() " + getMeasuredHeight());
        if (!mHasDrawn) {
            resetText();
        }
        super.onDraw(canvas);
    ...
    private void resetText() {
        ...
        if (mIsFold) {
            //要收缩状态
            if (mCanFoldAgain) {
                openText = createClickUnFoldSpan(mFullText).toString().trim();
                spanStr = createUnFoldSpan(mFullText);
                if (null != callback&&!TextUtils.isEmpty(openText)) {
                    callback.callBack(openText);
                }
            }
        } else {
            //要展开状态
            closeText = createClickFoldSpan(mFullText).toString().trim();
            spanStr = createFoldSpan(mFullText);
            if (null != callback&& !TextUtils.isEmpty(closeText)) {
                callback.callBack(closeText);
            }
        }
     ...
     * @return 展开状态下的Span
     */
    private SpannableString createClickUnFoldSpan(String text) {
        String destStr = text;
        int start = destStr.length();
        int end = destStr.length();

        SpannableString spanStr = new SpannableString(destStr);
        spanStr.setSpan(clickSpanContent, start, end, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
...
//        LogUtils.d(TAG, (System.currentTimeMillis() - startTime) + "ms");
     ...
     * @return 收缩状态下的Span
     */
    private SpannableString createClickFoldSpan(String text) {
        String destStr = tailorClickText(text);
        int start = destStr.length();
        int end = destStr.length();
        SpannableString spanStr = new SpannableString(destStr);
        spanStr.setSpan(clickSpanContent, start, end, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
...
//         LogUtils.d(TAG, "使用备用方法: tailorTextBackUp() " + mCountBackUp++);
     ...
     * @return 裁剪后的文本
     */
    private String tailorClickTextBackUp(String text) {
//         LogUtils.d(TAG, "使用备用方法: tailorTextBackUp() " + mCountBackUp++);
     ...
     * @return 裁剪后的文本
     */
    private String tailorClickText(String text) {
        //return tailorTextBackUp(text);
        ...
        String ret;
        if (find == 0) {
            ret = text.substring(0, mid);
        } else {
            ret = tailorClickTextBackUp(text);
        }
        return ret;
    }
