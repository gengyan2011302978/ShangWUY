...
public class TCControllerFullScreen extends RelativeLayout implements IController, View.OnClickListener,
     ...
     * V1.1.0新增
     */
    private ImageView mIvCollection;                //收藏按钮
    private ImageView mIvShare;                     //分享按钮
    private View mVodShare;                         //分享页面
    private ImageView mIvWechat;                    //微信分享按钮
    private ImageView mIvWechatCircle;              //微信朋友圈按钮
    private RecyclerView mRvMultiple;               //倍速弹框页面
    private TextView mTvMultiple;                   //底部倍速
    private TextView mTvDownload;                   //底部下载
    private ImageView mTvZoom;                      //底部缩放
    ...
    private void initView(Context context) {
        ...
        mVodShare = findViewById(R.id.vodShare);
        mIvWechat = findViewById(R.id.iv_wechat);
        mIvWechatCircle = findViewById(R.id.iv_wechat_circle);
        ...
        mTvZoom = findViewById(R.id.iv_zoom);
    ...
    private void toggle() {
        ...
        if (mVodShare.getVisibility() == VISIBLE) {
            mVodShare.setVisibility(GONE);
        }
        if (mRvMultiple.getVisibility() == VISIBLE) {
            mRvMultiple.setVisibility(GONE);
        }
    }
    ...
    public void onClick(View view) {
        int i = view.getId();
        if (i == R.id.iv_back || i == R.id.tv_title || i == R.id.iv_zoom) { //顶部标题栏 或者 底部缩放按钮
            if (mControllerCallback != null) {
        ...
        } else if (i == R.id.tv_quality) { //画质按钮
            showQualityView();

        } else if (i == R.id.iv_lock) { //锁屏按钮
            toggleLockState();
        ...
        } else if (i == R.id.iv_collection) { //收藏按钮
            if (mControllerCallback != null) {
            ...
            if (mControllerCallback != null) {
                mControllerCallback.onShowMore(2);
            }

        } else if (i == R.id.iv_wechat) { //微信分享
            if (mControllerCallback != null) {
                mControllerCallback.onWeChatCallBack(0, "微信");
            }

        } else if (i == R.id.iv_wechat_circle) { //微信朋友圈分享
            if (mControllerCallback != null) {
                mControllerCallback.onWeChatCallBack(1, "朋友圈");
            }

        } else if (i == R.id.tv_multiple) { //底部倍速
            if (mControllerCallback != null) {
                mControllerCallback.onMultiple(2);
            }

        } else if (i == R.id.tv_download) { //底部下载
            if (mControllerCallback != null) {
                mControllerCallback.onDownload(2);
            }
    ...
    public void showShareView() {
        hide();
        mVodShare.setVisibility(VISIBLE);
    }

    /**
     * V1.1.0 显示倍速
     */
    public void showMultipleView(List<TCMultipleBean> multipleBeanList) {
        hide();
        mRvMultiple.setVisibility(VISIBLE);
