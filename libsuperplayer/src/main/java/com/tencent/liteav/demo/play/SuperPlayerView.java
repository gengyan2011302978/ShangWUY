...
public class SuperPlayerView extends RelativeLayout implements ITXVodPlayListener, ITXLivePlayListener {
    ...
    public TCControllerFullScreen mControllerFullScreen;          // 全屏模式控制view
    public TCControllerWindow mControllerWindow;              // 窗口模式控制view
    ...
    public TXVodPlayer mVodPlayer;                     // 点播播放器
        ...
        if (model.videoId != null || model.videoIdV2 != null) { // 根据FileId播放
            mCurrentProtocol.sendRequest(new IPlayInfoRequestCallback() {
                ...
                public void onError(int errCode, String message) {
                    TXCLog.e(TAG, "onFail: errorCode = " + errCode + " message = " + message);
...
//                defaultVideoQuality = videoQualities.get(0);
    ...
    public interface TimeReport {
        ...
        //60s上报
        void submitLookTime(long currentTime, long countTime);

        //观看完成上报
        void submitEndTime(long currentTime, long countTime);

        //获取当前观看时间 和 总时间
        void getCurrentTime(long currentTime, long countTime);
    }

    public TimeReport mTimeReport;

    public void setTimeReport(TimeReport timeReport) {
        this.mTimeReport = timeReport;
    }

    /**
     * V1.0.2 记录上一次上报的时间，防止重复上报
     */
    ...
    private void updateVideoProgress(long current, long duration) {
        ...
        //每分钟进行一次时间上报
        if (current != 0 && current % 60 == 0 && mTimeReport != null && lastTime != currentTime) {
            lastTime = currentTime;
            mTimeReport.submitLookTime(currentTime, countTime);
        }

        if (mTimeReport != null) {
            mTimeReport.getCurrentTime(currentTime, countTime);
        }
    ...
    private IControllerCallback mControllerCallback = new IControllerCallback() {
        ...
        public void onSpeedChange(float speedLevel) {
            ...
            if (mPlayerViewCallback != null) {
                mPlayerViewCallback.onMultipleChange(speedLevel);
            }
        }
        ...
        public void onShowMore(int type) {
            if (mPlayerViewCallback != null) {
                mPlayerViewCallback.onShowMore(type);
            }
        }
        ...
        public void onWeChatCallBack(int statusId, String name) {
            if (mPlayerViewCallback != null) {
                mPlayerViewCallback.onWeChatCallBack(statusId, name);
            }
        }
        ...
        public void onMultiple(int type) {
            if (mPlayerViewCallback != null) {
                mPlayerViewCallback.onMultiple(type);
            }
        }
        ...
        public void onDownload(int type) {
            if (mPlayerViewCallback != null) {
                mPlayerViewCallback.onDownload(type);
            }
        }
        ...
        public void onPlayEvent(boolean isPause) {
            if (mPlayerViewCallback != null) {
                mPlayerViewCallback.onPlayEvent(isPause);
            }
        }
        ...
        public void onCollection() {
            if (mPlayerViewCallback != null) {
                mPlayerViewCallback.onCollection();
            }
        }
    };
    ...
    public void onPlayEvent(TXVodPlayer player, int event, Bundle param) {
        ...
        switch (event) {
            case TXLiveConstants.PLAY_EVT_VOD_PLAY_PREPARED://视频播放开始
                //V1.0.2 设置播放开始时间
                mControllerCallback.onSeekTo((int) startPlayTime);
                ...
                if (mIsMultiBitrateStream) {
                    ...
                    //masterPlaylist多清晰度，按照码率排序，从低到高
            ...
            case TXLiveConstants.PLAY_EVT_PLAY_END:
                //V1.0.2 重新定位播放时间，上报播放时长
                startPlayTime = 0;
                if (mTimeReport != null) {
                    mTimeReport.submitEndTime(currentTime, countTime);
                }
    ...
    public interface OnSuperPlayerViewCallback {
         ...
         * 点击右上角更多
         */
        void onShowMore(int type);

        /**
         * 分享按钮 微信
         */
        void onWeChatCallBack(int statusId, String name);

        /**
         * 倍速
         */
        void onMultiple(int type);

        /**
         * 倍速修改
         */
        void onMultipleChange(float speedLevel);
         ...
         * 手动暂停 播放
         */
        void onPlayEvent(boolean isPause);
    }
